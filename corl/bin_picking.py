...
N_TASKS = 2
EXP_PREFIX = 'corl_te_bin_picking'


def run_task(v):

    goal_low = np.array((0.12, 0.7, 0.02))
    goal_high = np.array((0.12, 0.7, 0.02))
    ...
    TASKS = {
        str(i + 1): {
            "args": [],
            "kwargs": {
                'tasks': [{'goal': np.array(g), 'obj_init_pos': np.array([-0.12, 0.7, 0.02]), 'obj_init_angle': 0.3}],
                'random_init': False,
            }
        }
    ...
    with TaskEmbeddingRunner() as runner:
        ...
        env = TfEnv(
                MultiTaskEnv(
                    task_env_cls=SawyerBinPicking6DOFEnv,
                    task_args=task_args,
